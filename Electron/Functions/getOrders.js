 ...
 */
async function getOrders(esiClient, buySell, itemID, array, fedName) {
    ...
    async function fetch() {
        ...
        let data;
        try {
            data = await esiClient.market.orders(regID.id, itemID, buySell, 1)
            data = data.data
        } catch (error) {
        ...
        for (let i = 0; i < data.length; i++, j++) {
            ...
            try {
                currentData = data[i]
                if (`${currentData.location_id}`.length > 8) { // Structures ID is longer than 8
                    return
                }
                let search = await esiClient.universe.stations.stationInfo(currentData.location_id)
                search = search.data
          ...
          <th>Remaining/Minimum Volume</th>`
            ...
            await esiClient.util.sleep(50) // pause on each iteration to avoid spamming the ESI Server
        }
